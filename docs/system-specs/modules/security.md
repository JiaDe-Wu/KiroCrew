...
a `$` anywhere in any token lands the segment on the ungated branch (parameter
...
separator upstream, only `&&` is) or operator glue mid-word (`main>log` is the
word `main` plus a redirection — it pushes main) marks the split untrusted AND
scans the operator-delimited pieces, so a protected name cannot hide behind
glue. Quoted operator characters are data and none of this fires. A segment
