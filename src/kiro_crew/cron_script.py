...
def _pin_digest(payload: bytes, *, domain: str, job_id: str) -> str:
    ...
    epoch_payload = b"epoch\x00" + str(_grant_epoch(job_id)).encode() + b"\x00" + payload
    return _hmac.new(_grant_pin_key(), epoch_payload, hashlib.sha256).hexdigest()
...
def _grant_epoch(job_id: str) -> int:
    ...
    gateway's own grant paths can bump an epoch.
    """
    try:
        data = json.loads(_grant_epochs_path().read_text())
...
def bump_grant_epoch(job_id: str) -> int:
    ...
    Called by every owner grant WRITE (grant, replace, promote, revoke)
    before minting, so no pin from an earlier grant state — including one the
    agent copied out of the store while it was valid — survives the write.
    """
    ...
    try:
        ...
        data = {}
    data[job_id] = int(data.get(job_id, 0)) + 1
...
def compute_secret_env_pin(
    ...
    *,
    ...
    return _pin_digest(
        _pin_payload(script, command, message, body, job_id, grant, domain),
        domain=domain,
        job_id=job_id,
    )
...
def run_script_sandboxed(
    ...
    if resolved_secret_env:
        ...
        stdin_payload = json.dumps(
            {
                "body_b64": _b64.b64encode(script_body or b"").decode(),
    ...
    try:
        ...
        # sys.path. Ungranted scripts keep their normal view.
        hidden = (str(config_dir() / "crons"),) if stdin_payload is not None else ()
        sandboxed_argv, sandbox_cleanup = wrap_argv(argv, mode="standard", extra_hidden_dirs=hidden)
