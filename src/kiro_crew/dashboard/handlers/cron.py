...
async def _promote_pending_grant(
    ...
    try:
        ...
        def _both_pins() -> tuple[str, str]:
            ...
            # Bump the grant epoch BEFORE minting: every previously minted
            # active pin (including one the agent copied out of the store
            # while valid) dies with the old epoch, so approval is also an
            # implicit revoke of any prior grant state.
...
async def api_cron_secret_grant(request: web.Request) -> web.Response:
    ...
    if secret_env:
        if not job.script:
            return web.json_response(
                {
                    "error": "secret grants apply only to SCRIPT jobs — "
                    "an agent job would expose plaintext to the model; a command "
                    "job's pin cannot cover invoked helper files",
                    "code": "invalid_secret_env",
                },
                status=400,
            )
        ...
        try:
            ...
            # The epoch bump first invalidates every previously minted pin —
            # a replace is an implicit revoke of the prior grant state.
            def _mint() -> str:
                bump_grant_epoch(job.id)
                return compute_secret_env_pin(
                    job.script,
                    job.command,
                    job.message,
                    job_id=job.id,
            ...
            pin = await asyncio.to_thread(_mint)
        except (ValueError, FileNotFoundError, PermissionError, RuntimeError) as exc:
            return web.json_response({"error": str(exc), "code": "invalid_secret_env"}, status=400)
    else:
        # Revoke: bump the epoch so a saved copy of the old mapping+pin
        # written back into the agent-writable store never verifies again.
