...
class TestSeedHomeFromScenario:
    ...
    def test_a_non_directory_home_refuses_instead_of_crashing(
        ...
        with pytest.raises(rt.PodError, match="is not a directory"):
...
class TestSeededScenarioInHome:
    ...
    def test_names_the_scenario_a_seeded_home_holds(self, tmp_path: Path, monkeypatch) -> None:
        monkeypatch.setenv("KIROCREW_POD_ROOT", str(tmp_path / "pods"))
        cfg = PodConfig.load()
        rt.seed_home_from_scenario(cfg, "wt", "crons-active")
        assert rt.seeded_scenario_in_home(cfg, "wt") == "crons-active"
...
class TestSanitizeHomeConfig:
    ...
    def test_a_failed_rewrite_refuses_instead_of_leaving_channels_enabled(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        """A write that cannot land must abort the boot, not fall through.

        The read-failure paths above are tolerated because there is nothing to
        tighten. Here the roster is known to be present and was NOT disabled, and
        the boot argv's `--no-tunnel` does not cover the sharpest section on it:
        WhatsApp reconnects through the operator's own account paired as a linked
        device, which no tunnel flag gates. Failing open here boots a pod live on
        the operator's real number.
        """
        (tmp_path / "config.json").write_text(json.dumps({"whatsapp": {"enabled": True}}))

        def _refuse(*args: object, **kwargs: object) -> None:
            raise OSError("read-only file system")

        monkeypatch.setattr(rt, "atomic_write", _refuse)
        with pytest.raises(rt.PodError, match="could not disable the seeded channel sections"):
            rt.sanitize_home_config(tmp_path)
